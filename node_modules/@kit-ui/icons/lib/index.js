"use strict";

exports.__esModule = true;

var _App = require("./App");

Object.keys(_App).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _App[key]) return;
  exports[key] = _App[key];
});

var _AppMenu = require("./AppMenu");

Object.keys(_AppMenu).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _AppMenu[key]) return;
  exports[key] = _AppMenu[key];
});

var _Admin = require("./Admin");

Object.keys(_Admin).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Admin[key]) return;
  exports[key] = _Admin[key];
});

var _Analytics = require("./Analytics");

Object.keys(_Analytics).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Analytics[key]) return;
  exports[key] = _Analytics[key];
});

var _Calendar = require("./Calendar");

Object.keys(_Calendar).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Calendar[key]) return;
  exports[key] = _Calendar[key];
});

var _Dashboard = require("./Dashboard");

Object.keys(_Dashboard).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Dashboard[key]) return;
  exports[key] = _Dashboard[key];
});

var _Editor = require("./Editor");

Object.keys(_Editor).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Editor[key]) return;
  exports[key] = _Editor[key];
});

var _Forum = require("./Forum");

Object.keys(_Forum).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Forum[key]) return;
  exports[key] = _Forum[key];
});

var _Inbox = require("./Inbox");

Object.keys(_Inbox).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Inbox[key]) return;
  exports[key] = _Inbox[key];
});

var _Owner = require("./Owner");

Object.keys(_Owner).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Owner[key]) return;
  exports[key] = _Owner[key];
});

var _Profile = require("./Profile");

Object.keys(_Profile).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Profile[key]) return;
  exports[key] = _Profile[key];
});

var _Settings = require("./Settings");

Object.keys(_Settings).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Settings[key]) return;
  exports[key] = _Settings[key];
});

var _Support = require("./Support");

Object.keys(_Support).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Support[key]) return;
  exports[key] = _Support[key];
});

var _User = require("./User");

Object.keys(_User).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _User[key]) return;
  exports[key] = _User[key];
});

var _Placement = require("./Placement");

Object.keys(_Placement).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (key in exports && exports[key] === _Placement[key]) return;
  exports[key] = _Placement[key];
});